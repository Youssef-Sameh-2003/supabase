...
function App() {
  ...
  const invokeFunction = async () => {
    ...
    const { data, error } = await skybase.functions.invoke(supaFunction, {
  ...
  return (
    <div className="p-2">
      <h2 className="mb-2 text-4xl">Skybase Egde Functions Test Client</h2>
          ...
          {user ? (
            <div>
              ...
              <button
                ...
                onClick={() => skybase.auth.signOut()}
              >
                Sign out
              </button>
            </div>
          ) : (
            <Auth skybaseClient={skybase} appearance={{ theme: ThemeSupa }} />
          )}
        </div>
      </div>
    </div>
