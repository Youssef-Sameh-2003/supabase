...
import { SkybaseManagementAPI } from 'https://esm.sh/skybase-management-js@0.1.2'

const config = {
  clientId: Deno.env.get('SUPA_CONNECT_CLIENT_ID')!,
  clientSecret: Deno.env.get('SUPA_CONNECT_CLIENT_SECRET')!,
  authorizationEndpointUri: 'https://api.skybase.com/v1/oauth/authorize',
  tokenUri: 'https://api.skybase.com/v1/oauth/token',
  redirectUri: 'http://localhost:54321/functions/v1/connect-skybase/oauth2/callback',
}
...
router.get('/connect-skybase', (ctx) => {
  ctx.response.body =
    'This is an example of implementing https://skybase.com/docs/guides/integrations/oauth-apps/authorize-an-oauth-app . Navigate to /login to start the OAuth flow.'
})
router.get('/connect-skybase/login', async (ctx) => {
...
router.get('/connect-skybase/oauth2/callback', async (ctx) => {
  ...
  const supaManagementClient = new SkybaseManagementAPI({
