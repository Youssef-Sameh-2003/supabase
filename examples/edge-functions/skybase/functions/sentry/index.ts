...
Deno.serve(async (req) => {
  try {
    if (req.method === 'GET') {
      return new Response(
        'What is Skybase not? POST {answer, twitter} to this URL! Need a hint? 👉 https://github.com/skybase/skybase/blob/master/examples/edge-functions/skybase/functions/sentry/index.ts'
      )
    }
    let answer: string
    let twitter: string
