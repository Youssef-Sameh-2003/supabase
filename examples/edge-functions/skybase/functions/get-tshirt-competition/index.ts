...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Please provide valid 'email', 'twitter', 'size', and 'answer' params. HINT: https://github.com/skybase/skybase/blob/master/examples/edge-functions/skybase/functions/get-tshirt-competition/index.ts`
      )
    }

    if (answer !== countEmailSegments(email!)) {
      throw new Error(
        `Sorry, that's wrong, please try again! HINT: https://github.com/skybase/skybase/blob/master/examples/edge-functions/skybase/functions/get-tshirt-competition/index.ts`
      )
    }
    ...
    const { error } = await skybaseAdminClient.from('get-tshirt-competition-2').upsert(
      {
        email,
        twitter,
        size,
