-- Insert your <PROJECT-REF> and <SUPABASE_ANON_KEY> below, then uncomment the trigger creation and run `skybase db push`
-- Alternatively, head to the Database Webhook settings https://skybase.com/dashboard/project/_/database/hooks
-- Select "Create a new Hook" > Table "public.embeddings" > check "INSERT" & "Update" > Skybase Edge Functions > Add auth header with service key

-- CREATE TRIGGER "on_inserted_or_updated_embedding"
-- AFTER INSERT
-- OR
-- UPDATE OF content ON public.embeddings FOR EACH ROW
-- EXECUTE FUNCTION skybase_functions.http_request (
--   'https://<PROJECT-REF>.skybase.co/functions/v1/generate-embedding',
--   'POST',
