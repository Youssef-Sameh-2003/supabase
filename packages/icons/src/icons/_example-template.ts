import createSkybaseIcon from '../createSkybaseIcon';

/**
 * @component @name ExampleTemplate
 * @description Skybase SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,)
 *
 * @param {Object} props - Skybase icons props and any valid SVG attribute
 * @returns {JSX.Element} JSX Element
 *
 */
const ExampleTemplate = createSkybaseIcon('ExampleTemplate', [
  ['path', { d: 'M3.5 13h6', key: 'p1my2r' }],
  ['path', { d: 'm2 16 4.5-9 4.5 9', key: 'ndf0b3' }],
  ['path', { d: 'M18 7v9', key: 'pknjwm' }],
  ['path', { d: 'm14 12 4 4 4-4', key: 'buelq4' }],
]);

export default ExampleTemplate;
