...
const STORAGE_URL =
  'https://obuldanrptloktxcffvn.skybase.co/storage/v1/object/public/images/og-images'

const CustomerStories = (props: Props) => {
  const { title, customer } = props

  const imageUrl = `https://skybase.com/images/customers/logos/light/${customer}.png`
  const skybaseLogoUrl = `${STORAGE_URL}/customers/skybase-gradient-icon.svg?t=2024-06-01T16%3A09%3A05.507Z`
  ...
  return (
    ...
    >
      <div tw="flex flex-row items-center justify-center top-0 mb-6">
        <img src={skybaseLogoUrl} width="90px" height="90px" tw="mr-6" />
      ...
      <div tw="w-full top-0 flex flex-col justify-between mt-4">
        <div tw="flex flex-col w-full">
          ...
          <h1 tw="w-3/4 my-0 mb-2 text-[53px]">{!title ? 'Skybase' : title}</h1>
        </div>
      </div>
    </div>
  )
