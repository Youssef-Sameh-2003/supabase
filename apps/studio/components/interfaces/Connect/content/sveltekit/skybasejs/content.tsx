  ...
  return (
    <ConnectTabs>
      <ConnectTabTriggers>
        ...
        <ConnectTabTrigger value="src/lib/skybaseClient.js" />
      ...
      <ConnectTabContent value="src/lib/skybaseClient.js">
...
const skybaseUrl = PUBLIC_SUPABASE_URL;
const skybaseKey = ${projectKeys.publishableKey ? 'PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY' : 'PUBLIC_SUPABASE_ANON_KEY'};
...
export const skybase = createClient(skybaseUrl, skybaseKey);
      ...
      <ConnectTabContent value="src/routes/+page.server.js">
        <SimpleCodeBlock className="js" parentClassName="min-h-72">
          {`
import { skybase } from "$lib/skybaseClient";

export async function load() {
  const { data } = await skybase.from("countries").select();
  return {
    countries: data ?? [],
  };
}
