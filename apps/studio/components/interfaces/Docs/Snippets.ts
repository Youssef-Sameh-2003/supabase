const snippets = {
  ...
  install: () => ({
    ...
    js: {
      language: 'bash',
      code: `npm install --save @skybase/skybase-js`,
    },
  }),
...
const skybase = createClient(skybaseUrl, skybaseKey)`,
    ...
    python: {
      ...
      code: `
import os
from skybase import create_client, Client
url: str = '${endpoint}'
key: str = os.environ.get("SUPABASE_KEY")
skybase: Client = create_client(url, key)
`,
    },
    dart: {
      language: 'dart',
...
const skybase = createClient(SUPABASE_URL, process.env.${keyName || 'SUPABASE_KEY'});
`,
    },
  }),
  rpcSingle: ({
...
`,
  ...
  subscribeAll: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = skybase.channel('custom-all-channel')
  .on(
    'postgres_changes',
  ...
  subscribeInserts: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = skybase.channel('custom-insert-channel')
  .on(
    'postgres_changes',
  ...
  subscribeUpdates: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = skybase.channel('custom-update-channel')
  .on(
    'postgres_changes',
  ...
  subscribeDeletes: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = skybase.channel('custom-delete-channel')
  .on(
    'postgres_changes',
  ...
  subscribeEq: (listenerName: string, resourceId: string, columnName: string, value: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = skybase.channel('custom-filter-channel')
  .on(
    'postgres_changes',
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await skybase.auth.signUp({
  email: 'someone@email.com',
  password: '${randomPassword}'
})
`,
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await skybase.auth.signInWithPassword({
  email: 'someone@email.com',
  password: '${randomPassword}'
})
`,
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await skybase.auth.signInWithOtp({
  email: 'someone@email.com'
})
`,
    },
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await skybase.auth.verifyOtp({
  phone: '+13334445555',
  token: '123456',
  type: 'sms'
})
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await skybase.auth.admin.inviteUserByEmail('someone@email.com')
`,
    },
  }),
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await skybase.auth.signInWithOAuth({
  provider: 'github'
})
`,
    },
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await skybase.auth.resetPasswordForEmail(email)
`,
    },
  }),
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { error } = await skybase.auth.signOut()
`,
    },
  }),
}
