...
const getTools = () => {
  return {
    ...
    setServices: tool({
      description:
        'Set the entire list of Skybase services needed for the project. Always include the full list',
      parameters: z.object({
        services: z
          .array(ServiceSchema)
...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  const result = await streamText({
    ...
    maxSteps: 7,
    system: source`
      You are a Skybase expert who helps people set up their Skybase project. You specializes in database schema design. You are to help the user design a database schema for their application but also suggest Skybase services they should use. 
      ...
      - In Skybase, the auth schema already has a users table which is used to store users
